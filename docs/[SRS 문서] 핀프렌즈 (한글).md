...
**문서 계열:** 기술 **중립판** — 무엇을 만들 것인가를 정의한다. 특정 스택으로 어떻게 성립시킬지는 병렬 문서 `[SRS 문서] 핀프렌즈 (기술제약 반영판).md`(SRS-FINFRIENDS-TEC-001)가 정의하며, **요구사항 ID는 두 문서가 공유**한다.

**입력 문서**
- `finfriends-prd-v1_0.md` — 핀프렌즈 PRD v1.0 (2026-08-24 · `new-rim/finfriends-prd-to-srs` 저장소)
...
>
> **서비스 경계의 지위** — 위 내부 서비스 9종은 **책임 경계**이며 배포 단위를 규정하지 않는다. 단일 애플리케이션의 모듈로 구현할지 독립 서비스로 배포할지는 **기술 선택의 문제**이며, 본 문서는 그것을 정하지 않는다. 특정 스택을 전제한 배치는 병렬 문서인 **기술제약 반영판**이 정의한다.
...
| **7** | **열려 있는 것 6건**(D1~D6)은 미결이 아니라 **확정 시점이 박힌 대기**다 — 계약 전 / 착수 전 / 제작 착수 전 |
| **8** | **기준선은 전부 실측 전**이다. §9.4의 PASS 값은 **목표이지 성과가 아니며**, 대외 인용 시 §1.4 ③의 금지선을 따른다 |
